...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала пробуем полный формат с датами
...
async def daily_digest(context: ContextTypes.DEFAULT_TYPE):
    ...
    per_qr: List[Tuple[str, Tuple[int, int, int]]] = []
    qr_ids = list(QR_IDS_ENV)

    try:
        if not qr_ids:
            data_all = await api_search_async(None, day_start, day_end)
            evs = data_all.get("events") or []
            qr_ids = extract_qr_ids_from_events(evs)

        results = await asyncio.gather(
            *(api_search_async(q, day_start, day_end) for q in qr_ids),
            return_exceptions=True,
        )
        for q, res in zip(qr_ids, results):
            if isinstance(res, Exception):
                log.warning("Daily: failed qr %s: %s", q, res)
                continue
            events = res.get("events") or []
            per_qr.append((q, summarize_counts(events)))

        lines = ["Статистика по всем QR за сегодня."]
        for qr_id, (visits, subs, unsubs) in per_qr:
